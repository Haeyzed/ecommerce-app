...
export const registerSchema = z
  .object({
    ...
    username: z
      .union([
        z.string().max(255).regex(/^[a-zA-Z0-9_-]*$/, 'Only letters, numbers, underscores and hyphens'),
        z.literal(''),
      ])
      .optional(),
    email: z
      ...
      .optional(),
    image: z.array(imageFileSchema).max(1, 'Please select only one image').optional(),
    phone: z.string().max(255, 'Phone number is too long').optional().or(z.literal('')),
  ...
  .refine((data) => (data.email ?? '').length > 0 || (data.username ?? '').length > 0, {
